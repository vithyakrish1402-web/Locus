...
const INITIAL_MOCK_USERS = [
  { id: 'u1', name: "Aryan_Web3", role: "Student", lat: 12.82480, lng: 80.04510, status: "In Library", permission: "none" },
  { id: 'u2', name: "Sanya.eth", role: "Faculty", lat: 12.82330, lng: 80.04260, status: "At UB Lab", permission: "accepted" },
  { id: 'u3', name: "SRM_Dev", role: "Staff", lat: 12.82125, lng: 80.04155, status: "Java Green", permission: "requested" },
  { id: 'u4', name: "CryptoWhiz", role: "Student", lat: 12.82315, lng: 80.04445, status: "Main Block", permission: "none" },
];

const CustomMarker = ({ isUser, name, onClick }) => {
  if (isUser) {
    return (
      <div 
        onClick={onClick}
        className="w-10 h-10 -ml-5 -mt-5 bg-gradient-to-br from-emerald-400/80 to-teal-500/80 backdrop-blur-md rounded-full border border-emerald-200/50 shadow-[0_0_15px_rgba(52,211,153,0.6)] flex items-center justify-center text-white font-bold tracking-tighter cursor-pointer"
      >
        {name.charAt(0)}
      </div>
    )
...
const CinematicLanding = ({ 
  ...
  // 7 scenes total + login = ~700vh height
  ...
  // SCENE 7: Event Navigation & FINAL: Auth Node
    ...
    <div ref={containerRef} className="relative w-full h-[800vh] bg-black text-white font-inter selection:bg-red-500/30">
      ...
      <div className="sticky top-0 w-full h-screen overflow-hidden flex items-center justify-center perspective-[1000px] bg-dots">
        ...
        <motion.div style={{ opacity: heroOpacity, scale: heroScale, y: 0 }} className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center z-10 w-full max-w-7xl mx-auto pointer-events-none">
           ...
           {/* Floating Particle Campus Abstract */}
...
const App = () => {
  ...
  const handleLogin = (method) => {
    if (method === 'email' && (!email || !password)) return;
    setLoginMethod(method);
    setTimeout(() => {
      setIsAuthenticated(true);
      setCurrentUser(method === 'google' ? 'Google Account' : email);
    }, 1800);
  };
  ...
  // --- LANDING PAGE: NOTHING BRAND AESTHETIC ---
  if (!isAuthenticated) {
    return (
      <CinematicLanding 
        email={email}
  ...
  // --- DASHBOARD: NOTHING THEME MAP LOGIC ---
  return (
                ...
                users.filter(u => !blockedUserIds.includes(u.id)).map(user => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 mb-4">
                      <div className={`w-10 h-10 flex items-center justify-center font-dot text-sm border ${user.permission === 'accepted' ? 'border-red-500 text-red-500' : 'border-white/20 text-zinc-500'}`}>
                        {user.name.charAt(0)}
                      </div>
                  ...
                  pendingRequests.length === 0 ? (
                    ...
                    pendingRequests.map(user => (
                      <div key={user.id} className="p-4 bg-black border border-white/20 flex flex-col gap-4 relative">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 border border-white/30 flex items-center justify-center font-dot text-xl text-zinc-400">
                             {user.name.charAt(0)}
                          </div>
                          <div>
